...
def save_tsdf_full(args, scene_path, cam_intr, depth_list, cam_pose_list, color_list, use_gpu=True, save_mesh=False):
    # ======================================================================================================== #
    ...
    if n_imgs > 200:
        ind = np.linspace(0, n_imgs - 1, 200).astype(np.int32)#select200
    ...
    for id in image_id:
        ...
        view_frust_pts = get_view_frustum(depth_im, cam_intr, cam_pose)#project cornder to 3D world
...
def save_fragment_pkl(args, scene, cam_intr, depth_list, cam_pose_list, imgfile_list):
    ...
    print('Completed: process scene {}'.format(scene))
...
def process_with_single_worker(args, scannet_files):
    ...
    for scene in tqdm(scannet_files):
        ...
        if args.dataset == 'scannet':
            ...
            dataset = ScanNetSceneDataset(scene, args.data_path, args.max_depth)
        # print(cam_intr)
...
def process_scannet(args, scannet_files):
    for scene in tqdm(scannet_files):
        depth_all = {}
        cam_pose_all = {}
        color_all = {}
        ...
        for id, (cam_pose, depth_im, img_file) in enumerate(dataloader):
            ...
            # color_all.update({id: color_image})
        use_gpu = True
        save_tsdf_full(args, scene, cam_intr, depth_all, cam_pose_all, color_all, use_gpu=use_gpu, save_mesh=True)
        save_fragment_pkl(args, scene, cam_intr, depth_all, cam_pose_all, imgfile_all)


def process_seven_scenes(args):
    #intrinsics, img_size=(480, 640)(h,w)
    cam_intr = np.array([[585, 0 , 320],
                        [0, 585, 240],
                        [0, 0, 1]])
    for scene in dataconfig.SEVEN_SCENES_TEST:
        dataset = SevenScenesSceneDataset(scene_path = f'{args.data_path}/{scene}', 
        ...
        for id, (cam_pose, depth_im, img_file) in enumerate(dataloader):
            if id % 100 == 0:
                print("{}: read frame {}/{}".format(scene, str(id), str(len(dataloader))))
            ...
            imgfile_all.update({id:img_file})
            # color_all.update({id: color_image})
        # print(os.path.join(args.save_path, scene))
        # import ipdb; ipdb.set_trace()
        save_tsdf_full(args, scene, cam_intr, depth_all, cam_pose_all, color_all, save_mesh=False)
        save_fragment_pkl(args, scene, cam_intr, depth_all, cam_pose_all, imgfile_list = imgfile_all)
...
def generate_pkl_scannet(args):
    all_scenes = sorted(os.listdir(args.save_path))
    datapath = '/'.join(args.data_path.split('/')[:-1])
    ...
    for split in splits:
        fragments = []
        with open(os.path.join(datapath, 'splits', 'scannetv2_{}.txt'.format(split))) as f:
...
def generate_pkl_7scenestest(args):
    # all_scenes = sorted(os.listdir(args.save_path))
    all_scenes = dataconfig.SEVEN_SCENES_TEST
    # datapath = '/'.join(args.data_path.split('/')[:-1])
    ...
    with open(os.path.join(args.save_path, 'fragments_test.pkl'), 'wb') as f:
        pickle.dump(fragments, f)
...
if __name__ == "__main__":
    ...
    if args.dataset == 'scannet':
        ...
        if not args.test:
            args.data_path = cfg.TRAIN.PATH
        else:
            args.data_path = cfg.TEST.PATH
        files = sorted(os.listdir(args.data_path))
        if all_proc ==1:
            process_scannet(args, files)
            generate_pkl_scannet(args)
        else:
            ...
            #aggregate all tr/val/test into one fragments.pkl
            generate_pkl_scannet(args)
    elif args.dataset == '7scenes': #one-process
        process_seven_scenes(args)
        generate_pkl_7scenestest(args)
    else:
