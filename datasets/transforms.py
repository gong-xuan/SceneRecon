...
from utils.utils import coordinates
    ...
    def __call__(self, data):
        ...
        for intrinsics, extrinsics in zip(data['intrinsics'], data['extrinsics']):
            ...
            for i in range(3):
                ...
                # print("=========",self.stride, extrinsics.dtype, intrinsics.dtype)
...
class RandomTransformSpace(object):
    ...
    def __init__(self, voxel_dim, voxel_size, random_rotation=True, random_translation=True,
        ...
        #
        self.vol_org_z = vol_org_z
        self.trunc_sdf = trunc_sdf
    ...
    def transform(self, data, transform=None, old_origin=None,
        ...
        num_layers = 3
        # print('================', (bnds[2, 1] + bnds[2, 0]) / 2)
        # import ipdb; ipdb.set_trace()
        # print(self.vol_org_z)
        if not self.vol_org_z=='None':
            # center = (torch.tensor(((bnds[0, 1] + bnds[0, 0]) / 2, (bnds[1, 1] + bnds[1, 0]) / 2, (bnds[2, 1] + bnds[2, 0]) / 2)) - data[
            #     'vol_origin']) / self.voxel_size
            trans_z = float(self.vol_org_z)
            center = (torch.tensor(((bnds[0, 1] + bnds[0, 0]) / 2, (bnds[1, 1] + bnds[1, 0]) / 2, trans_z)) - data[
                'vol_origin']) / self.voxel_size
            ...
            trans_z = 'center'
            vol_origin_partial = torch.tensor(((bnds[0, 1] + bnds[0, 0]) / 2, (bnds[1, 1] + bnds[1, 0]) / 2, (bnds[2, 1] + bnds[2, 0]) / 2)) 
            vol_origin_partial -= torch.tensor(self.voxel_dim)*self.voxel_size / 2.
        # import ipdb; ipdb.set_trace()
        # vol_last_partial = vol_origin_partial + torch.tensor(self.voxel_dim)*self.voxel_size
        # print('=============', vol_origin_partial, vol_last_partial, 'RealBounds', bnds[:,0], bnds[:,1])
        # print('================', self.voxel_dim, self.voxel_size)#(96,96,96), 0.04
        # 
        # print('======axis move====', trans_z, self.max_depth)
        ...
        if 'tsdf_list_full' in data.keys():
            ...
            for l, tsdf_s in enumerate(data['tsdf_list_full']):
                ...
                vol_dim_s = torch.true_divide(torch.tensor(self.voxel_dim) , 2 ** l).trunc().int()
                ...
                occ_vol[(tsdf_vol < trunc_sdf-0.001) & (tsdf_vol > -trunc_sdf+0.001) & (weight_vol > 1)] = True
