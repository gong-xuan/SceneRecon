...
class NeuConV1Net(nn.Module):
    '''
    Coarse-to-fine network.
    '''
    def __init__(self, cfg):
        super(NeuConV1Net, self).__init__()
        self.cfg = cfg
    ...
    def forward(self, features, inputs, outputs):
        ...
        ###
        pre_depth = None
        feat_fine= torch.stack([feat[0] for feat in features])
        nview, _, feat_ch, _, _ = feat_fine.shape
        pool_kernel = [4,2,1]
        # ----coarse to fine----
        for i in range(self.cfg.N_LAYER):#(0,1,2)
            ...
            ks = pool_kernel[i]
            if ks!=1:
                feats = F.max_pool2d(feat_fine.reshape(bs*nview, *feat_fine.shape[2:]), ks)
                feats = feats.reshape(nview, bs, *feats.shape[1:])
            else:
                feats = feat_fine
            # import ipdb; ipdb.set_trace()
