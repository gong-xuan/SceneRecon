...
from models.modules2D import MsTensoCNN

class NeuConV3Net(nn.Module):
    '''
    Coarse-to-fine network.
    '''
    def __init__(self, cfg):
        super(NeuConV3Net, self).__init__()
        self.cfg = cfg
        ...
        if self.cfg.AGG_3DV=='sim':
            ...
            gru_ch = [cfg.TENSO_CH*3, cfg.TENSO_CH*3+2, 3*cfg.TENSO_CH+2]
        ...
        # sparse conv
        self.convs= MsTensoCNN(in_channels=flatten_ch_in[0],
    ...
    def process_3dfeat(self, inputs, feat2d):
        ...
        for i in range(3):#(0,1,2)
            ...
            if i==0:
                out_feat = out_feat.reshape(bs, self.cfg.TENSO_CH*3, *n_vox)
            elif i==1:
                out_feat = out_feat.reshape(bs, self.cfg.TENSO_CH*3, n_vox[1],  n_vox[0],  n_vox[2]).permute(0,1,3,2,4)
            elif i==2:
                out_feat = out_feat.reshape(bs, self.cfg.TENSO_CH*3, n_vox[2],  n_vox[0],  n_vox[1]).permute(0,1,3,4,2)
            ##############
    ...
    def forward(self, features, inputs, outputs):
        ...
        feat_fine, grid_mask_fine = self.process_3dfeat(inputs, feat_fine2D) #(bs, ch*3, 96, 96, 96)
        # import ipdb; ipdb.set_trace()
        # ----coarse to fine----
        for i in range(self.cfg.N_LAYER):#(0,1,2)
            ...
            volume = volume.permute(1,0,2,3,4).reshape(self.cfg.TENSO_CH*3,-1).permute(1,0)
            if i != 0:
