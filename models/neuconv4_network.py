...
class NeuConV4Net(nn.Module):
    '''
    Coarse-to-fine network.
    '''
    def __init__(self, cfg):
        super(NeuConV4Net, self).__init__()
        self.cfg = cfg
        ...
        if self.cfg.AGG_3DV=='sim':
            ...
            flatten_ch_in = [(ch_agg + cfg.CONCAT_IMZ)*flatten_dim for flatten_dim in [6,12,24]] #24 h/w/d
            channels = [cfg.TENSO_CH*flatten_dim for flatten_dim in [6,12,24]]
        ...
        for i in range(3):
            self.convs.append(TensoCNN(in_channels=flatten_ch_in[i],
    ...
    def process_3dfeat(self, inputs, feat2d_fine, scale= 0):
        nview, bs = feat2d_fine.shape[:2]

        #scale
        grid_interval = 2**scale #1,2,4
        if scale==0:
            feat2d = feat2d_fine            
        else:
            feat2d = F.max_pool2d(feat2d_fine.reshape(bs*nview, *feat2d_fine.shape[2:]), grid_interval)
            feat2d = feat2d.reshape(nview, bs, *feat2d.shape[1:])
        # import ipdb; ipdb.set_trace()
        VOXEL_SIZE = grid_interval*self.cfg.VOXEL_SIZE#0.04, 0.08, 0.16
        N_VOX = [n//grid_interval for n in self.cfg.N_VOX]#96,48,24
        ...
        for i in range(3):#(0,1,2)
            interval = [grid_interval for _ in range(3)]
            ...
            # import ipdb; ipdb.set_trace()
            out_feat = self.convs[2-scale](in_feat)
            if i==0:
            ...
            out_feat = F.interpolate(out_feat, size=N_VOX, mode=self.cfg.UP)
            count = F.interpolate(count.float(), size=N_VOX, mode='nearest')
            feat.append(out_feat)
            grid_mask.append(count)
        feat = torch.stack(feat, dim=1).sum(dim=1)
        if self.cfg.NEW_GRID_MASK:
            grid_mask = back_project_visible(inputs['vol_origin_partial'], VOXEL_SIZE, N_VOX, KRcam, feat2d)
            grid_mask = (grid_mask>1).reshape(bs, *N_VOX) #(bs, D*H*W)
            # import ipdb; ipdb.set_trace()
    ...
    def forward(self, features, inputs, outputs):
        ...
        for i in range(self.cfg.N_LAYER):#(0,1,2)
            ...
            volume, grid_mask= self.process_3dfeat(inputs, feat_fine2D, scale) #(bs, ch, 24/48/96, 24/48/96, 24/48/96)
            ...
            # import ipdb; ipdb.set_trace()
            if i==0:
                if occupancy.shape[0]!=bs*voxel_size[0]*voxel_size[1]*voxel_size[2]:
                    new_mask = torch.zeros((bs, *voxel_size)).bool().to(occupancy.device)
                    new_mask[up_coords[:,0],up_coords[:,1]//2**(2-i), up_coords[:,2]//2**(2-i), up_coords[:,3]//2**(2-i)] = occupancy 
