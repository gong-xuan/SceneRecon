...
import torch
from utils.distort_utils import undistort_cam_pts_np, distort_cam_pts_np
...
class TSDFVolume:
    ...
    def integrate(self, color_im, depth_im, cam_intr, cam_pose, obs_weight=1., distCoef=None):
        ...
        if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
            ...
            cam_pts = rigid_transform(cam_pts, np.linalg.inv(cam_pose))#cam_pose is cam2world
            pix_z = cam_pts[:, 2]
            if distCoef is not None:
                cam_pts_xyz = distort_cam_pts_np(cam_pts.transpose(1,0), distCoef)
                cam_pts = cam_pts_xyz.transpose(1,0)
                # cam_pts = np.concatenate([cam_pts_xy, pix_z[:,None].copy()], axis=1)
    ...
    def get_mesh(self, scene_path):
        """Compute a mesh from the voxel volume using marching cubes.
        """
        tsdf_vol, color_vol, weight_vol = self.get_volume()
        print('TSDF:', tsdf_vol.min(), tsdf_vol.max(), scene_path)
        #
...
def get_view_frustum(depth_im, cam_intr, cam_pose, distCoef=None):
    ...
    if distCoef is None:
        ...
        cam_org_pts = np.zeros((3,1))  
        view_frust_pts2d = np.array([
            (np.array([0, 0, im_w, im_w]) - cam_intr[0, 2]) / cam_intr[0, 0],
            (np.array([0, im_h, 0, im_h]) - cam_intr[1, 2]) / cam_intr[1, 1]]) #(2,4)
        view_frust_pts2d = undistort_cam_pts_np(view_frust_pts2d, distCoef)
        view_frust_pts3d = np.concatenate([view_frust_pts2d, np.ones((1,4))], axis=0) * max_depth #TODO
        view_frust_pts = np.concatenate([cam_org_pts, view_frust_pts3d], axis=1)
